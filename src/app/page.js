...
const generateSampleListings = (count) => {
  ...
  for (let i = 1; i <= count; i++) {
    sampleListings.push({
      ...
      propertyType: ['apartment', 'house', 'studio'][Math.floor(Math.random() * 3)],
      bedrooms: Math.floor(Math.random() * 3) + 1,
      bathrooms: Math.floor(Math.random() * 2) + 1,
      images: ['/sample-home-image.jpg', '/sample-home-image-2.jpg'],
      amenities: ['Parking', 'Wifi', 'Gym', 'Pool'].slice(0, Math.floor(Math.random() * 4) + 1),
    });
  }
...
export default function Home() {
  ...
  const handleNewListing = (formData) => {
    ...
    const newListing = {
      ...
      ...formData,
      landlordId: user.id,
      images: formData.images.length > 0 ? [placeholderImageUrl, placeholderImageUrl] : [placeholderImageUrl]
    };
  ...
  const handleSearch = (searchCriteria) => {
    const filtered = listings.filter(listing => 
      ...
      (searchCriteria.propertyType ? listing.propertyType === searchCriteria.propertyType : true) &&
      (searchCriteria.bedrooms ? listing.bedrooms >= searchCriteria.bedrooms : true)
    );
    ...
    <div className="min-h-screen flex flex-col font-[family-name:var(--font-geist-sans)] bg-gray-50">
      <header className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:block">
            <ul className="flex space-x-8">
              <motion.li whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                <Link href="/" className="hover:text-yellow-300 transition-colors text-lg">Home</Link>
              </motion.li>
              <motion.li whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                <Link href="/search" className="hover:text-yellow-300 transition-colors text-lg">Search</Link>
              </motion.li>
              {isAuthenticated && (
                <motion.li whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                  <button onClick={() => setShowNewListingForm(true)} className="hover:text-yellow-300 transition-colors text-lg">Post a Listing</button>
                </motion.li>
              )}
              <motion.li whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                <Link href="/about" className="hover:text-yellow-300 transition-colors text-lg">About</Link>
              </motion.li>
            </ul>
          </nav>
          <div className="flex items-center space-x-4">
            {isAuthenticated ? (
              <>
                <span className="hidden sm:inline text-lg">Welcome, {user.name}</span>
                <motion.button 
                  onClick={handleLogout} 
                  className="bg-white text-blue-600 px-6 py-2 rounded-full hover:bg-yellow-300 hover:text-blue-800 transition-colors shadow-md"
              ...
              <motion.button 
                ...
                className="bg-yellow-300 text-blue-800 px-6 py-2 rounded-full hover:bg-white hover:text-blue-600 transition-colors shadow-md"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Login/Register
      ...
      <main className="flex-grow container mx-auto p-8">
        <motion.section 
          ...
          transition={{ duration: 0.5 }}
        >
          <h2 className="text-2xl font-semibold mb-4 text-gray-800">Find Your Perfect Home</h2>
        ...
        {showNewListingForm && (
          ...
          >
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">Post a New Listing</h2>
            <NewListingForm onSubmit={handleNewListing} />
          </motion.section>
        )}

        <section className="listings-section">
          <h2 className="text-2xl font-semibold mb-6 text-gray-800">Available Homes</h2>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-gray-300">To Let Home Finder helps you find and list rental properties easily.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex space-x-4">
                ...
                <a href="#" className="text-pink-400 hover:text-pink-300 transition-colors">Instagram</a>
              </div>
            </div>
          </div>
